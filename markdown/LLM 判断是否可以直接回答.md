...
**规则**：
1. **名词提取**：逐一提取【输入】中的所有【具体产品/品类】，**严禁遗漏紧跟在数量词后的名词**。
2. **查字典**：对比【对话历史】。
    - **need_rag**：只要有任何一个名词没在出现在对话历史中，直接出此项（一票否决）。
    - **no_rag**：名词全都在对话历史里；或者没有新名词，只是在基于常识属性（如：有汤、冷热、份量、甜咸等）进一步筛选。
...
**判定规则**：
扫描对比【输入】与【对话历史】中的【具体产品/品类】。
- **need_rag**：输入中出现对话历史中**从未提及**的具体产品/品类（包括追加新商品）。注意不要遗漏名称，特别是跟在数量词后面的名称。
- **no_rag**：输入中不存在任何新产品/品类，且内容仅限于：
  - 属性/形态偏好（如：有汤、辣度、甜咸、冷热、日式、点心/轻食类）。
  - 分量/场景描述（如：吃不多、下午茶、分量小）。
  - 代词（这个、那个）或确认语气（好、可以）。
